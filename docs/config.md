...
其中 `telegram`、`lark`、`discord`、`qq`、`wxwork`、`weixin` 都不再是 `src/platforms` 下的内置注册项，而是由 extension 注册。这些 IM 平台均为**可选 extension**：源码仓库 `extensions/` 目录已含源码；npm 包 / 二进制发行包默认不内嵌（由 `extensions/embedded.json` 白名单决定），使用前请先执行 `iris ext install <name>`；Docker 镜像默认额外打包了 `telegram` 和 `lark`。配置键仍然写在 `platform.yaml` 的 `telegram:` / `lark:` / `discord:` / `qq:` / `wxwork:` / `weixin:` 下。
...
- 可用 `iris models add/list/get/remove/default` 直接管理 `llm.yaml`；不带子命令的 `iris models` 仍打开 TUI 配置界面
...
`summary.yaml` 控制 `/compact`（包括自动 compact）的摘要提示词和单次摘要输出预算：
...
userPrompt: |
  Please summarize the above conversation history and preserve the unfinished task state.
```

| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `maxOutputTokens` | `number` | `16384` | 单次 summary 请求的最大输出 token，仅影响 compact |
| `systemPrompt` | `string` | 内置提示词 | 总结模型的系统提示词 |
| `userPrompt` | `string` | 内置分节提示词 | 追加在待总结历史末尾的指令 |

输出预算规则：

- 已知总结模型的 `contextWindow` 时，实际硬上限还会收紧到窗口的 20%；
- 模型 `requestBody` 中更小的输出限制会继续生效，但更大的值不能抬高 compact 专用上限；
- 上限按单次 summary 请求传递，不修改 Provider 全局状态，也不影响主聊天请求；
- 默认摘要指令会附加约为硬上限 75% 的软长度目标，给必要的收尾空间；
- 分块摘要、局部摘要合并、流式与非流式 summary 使用同一套限制。
...
实际文件为 `~/.iris/configs/mcp.yaml`（或 `IRIS_DATA_DIR/configs/mcp.yaml`），文件内顶层直接写 `servers`。
...
`--transport http` 会保存为 Iris 内部使用的 `streamable-http`。如需给指定 Agent 写覆盖层，可加 `--agent <name>`。
...
`cron` extension 提供定时任务调度能力：在指定时间、固定间隔或一次性延迟后，后台拉起一个 Agent 执行预设指令。

常用 CLI：

```bash
# 配置调度器开关（写入 ~/.iris/configs/cron.yaml）
iris cron config enable
iris cron config disable
iris cron status

# 创建任务（写入 ~/.iris/extension-data/cron/cron-jobs.json）
...
```bash
iris cron add --agent my-agent reminder --type once --value 10m --instruction "提醒我喝水"
```
...
| scope | 任务文件 |
|---|---|
| 全局 | `~/.iris/extension-data/cron/cron-jobs.json` |
| Agent | `~/.iris/agents/<name>/extension-data/cron/cron-jobs.json`，如配置了 `dataDir` 则使用自定义目录 |

运行中的 cron extension 会轮询同步 `cron-jobs.json` 的变化；`cron.yaml` 的配置开关可能需要重启或触发配置热重载后应用。
