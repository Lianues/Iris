...
- 多平台：Console / Web / Headless Core / Discord（可选 extension） / Telegram（可选 extension） / 微信（WeChat，可选 extension） / 企业微信（WXWork，可选 extension） / 飞书（Lark，可选 extension） / QQ（NapCat，可选 extension）
...
也可以用纯命令行管理模型池、MCP 和定时任务，例如：`iris models list`、`iris mcp add --transport http exa https://mcp.exa.ai/mcp`、`iris cron add morning --type cron --value "0 9 * * *" --instruction "生成一条早安问候" --silent`。不带子命令的 `iris models` 仍会打开 TUI 配置界面。
...
可通过命令添加（写入 Iris 的运行时配置目录，不读取 Claude CLI 的配置）：
...
Cron 由 `cron` extension 提供，用于让 Iris 在指定时间、固定间隔或一次性延迟后自动执行后台任务。任务定义保存在运行时数据目录的 `extension-data/cron/cron-jobs.json`；默认全局路径为 `~/.iris/extension-data/cron/cron-jobs.json`。

```bash
# 启用/查看调度器配置（写入 ~/.iris/configs/cron.yaml）
...
如需给指定 Agent 写独立任务，可加 `--agent <name>`。如果 Iris 正在运行，任务文件变化会被 cron extension 轮询同步；`cron.yaml` 的配置开关可能需要重启或触发配置热重载后应用。
...
Telegram 由 `extensions/telegram/manifest.json` 在启动时自动注册，运行时入口为 `extensions/telegram/dist/index.mjs`。源码仓库默认包含该 extension，发行包需通过 `iris ext install telegram` 安装。
