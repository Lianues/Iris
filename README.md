...
Cron 由内嵌 `cron` extension 提供，用于让 Iris 在指定时间、固定间隔或一次性延迟后自动执行后台任务。任务定义保存在运行时数据目录的 `extension-data/cron/cron-jobs.json`；默认全局路径为 `~/.iris/extension-data/cron/cron-jobs.json`。
