...
async function preflightShellCommand(
  ...
  if (!deps || !classifierConfig?.enabled) {
    ...
    if (fallback === 'deny') {
      ...
      return rejectedCommand(command, '命令不在安全白名单中且分类器未启用，拒绝执行。请使用只读命令（如 Get-ChildItem, Get-Content, git status 等），或请用户确认后使用 force: true。');
    }
...
export function createShellTool(deps?: ShellToolDeps): ToolDefinition {
  return {
    ...
    preflight: (args, context) => preflightShellCommand(args, context, deps),
    declaration: {
      name: 'shell',
...
- force 无法绕过黑名单（如 format C:、Invoke-Expression 等绝对禁止的操作）。但在 tools.yaml 中开启 autoApproveAll 或 shell.autoApprove 后，黑名单将被关闭。`,
    ...
    handler: async (args, context) => {
      const preflight = await preflightShellCommand(args, context, deps);
